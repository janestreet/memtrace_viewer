...
(** Generate labels for an axis of a graph, given the range of values on that axis. Labels
    will be evenly spaced with a "nice" gap between them, where a nice number is
    [n * 10**k] where n is 1, 2, or 5, and all labels will be within the given range.
    (This is different from the usual "loose labels" algorithm, found for instance in
    "Nice Numbers for Graph Labels" in Graphics Gems vol. 1, which places the labels
    outside the range of the data. We want our time series to cover the whole x-axis, so
    the last label won't reach quite to the edge.) *)
