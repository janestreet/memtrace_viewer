...
let allocation_site_indicator =
  ...
  Node.span
    ~attr:
      (Attr.many [ Attr.class_ "loc-alloc-site-indicator"; Attr.title "Allocation site" ])
    [ Node.text "A" ]
;;
...
let format_dom ~call_sites loc =
  ...
  let special text =
    Node.span ~attr:(Attr.classes [ "loc"; "loc-special" ]) [ Node.text text ]
  in
  match loc with
  ...
  | Function _ ->
    ...
    then Node.span ~attr:(Attr.class_ "loc") [ Node.text defname ]
      ...
      in
      Node.span
        ~attr:(Attr.many [ Attr.class_ "loc" ])
        [ Node.textf "%s " defname; call_sites ])
