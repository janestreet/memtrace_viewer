...
let find_peak_allocations trace =
  let size_table = Obj_id_table.create () in
  ...
  Raw_trace.iter trace ~parse_backtraces:false (fun time event ->
    match event with
    | Alloc { obj_id; size; _ } ->
      Obj_id_table.add_exn size_table ~key:obj_id ~data:size;
      ...
      then (
        ...
        peak_time := time)
    | Collect obj_id ->
      let size = Obj_id_table.find_and_remove_exn size_table obj_id in
      current_total := Byte_units.(!current_total - size)
    | Promote _ -> ()
    | End -> ());
